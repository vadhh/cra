...
- `detector/risk_explainer.py` — fast, deterministic per-finding "Explain Mode" (no ML calls, so it runs on every request — unlike the slower Qwen L4 narrative mode, which it does not touch or replace). `explain_findings(layer1, jurisdiction, doc_type_label)` attaches an inline `explanation: {...}` object to every red flag and missing required clause in `layer1`, mirroring how `citation_db.annotate_layer1()` attaches `citations: [...]`. Six fields per explanation: `clause`, `reason`, `severity`, `suggested_correction`, `confidence`, `source_reference`. Reason/recommendation come from a hand-written table for regex red flags (`_RED_FLAG_GUIDANCE`) or straight from the finding/`clause_db` for keyword_db findings and missing clauses; confidence is a static per-detection-method score (regex 0.90, keyword_db 0.75, missing-clause 0.80), not a per-instance ML probability. Wired in `app.py._run_analysis` right after `_semantic_backfill` (so semantically-recovered clauses are correctly excluded from "missing" explanations) and before `layer3_score`.
