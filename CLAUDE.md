...
**P1 from the review** (track alongside P2 Quality below): clause-coverage matrix (CR-11); lawyer-reviewed benchmark set measuring precision/recall/false-missing by type+jurisdiction+language (CR-05/07/08); version scoring policies + show score-version/confidence/limits in reports (CR-03); ~~language-aware keyword matching + evidence spans (CR-06)~~ — **DONE** (evidence spans added and validated); package or remove `legal_mlp.pkl` (overlaps P2 #8). Also: ~~fix "100% recall" wording → "all targeted cases passed" (CR-05)~~.
